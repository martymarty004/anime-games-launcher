...
async fn is_installed(
    ...
    game.is_addon_installed(group_name, addon_name, addon_path, edition).await
...
async fn get_diff(
    ...
    game.get_addon_diff(group_name, addon_name, addon_path, edition).await
...
async fn get_download(
    ...
    game.get_addon_download(group_name, addon_name, edition).await
        .map_err(|err| Box::new(MainAppMsg::ShowToast {
...
async fn get_diff_or_download(
    ...
    if is_installed(game, group_name, addon_name, addon_path.as_ref(), edition).await? {
        get_diff(game, group_name, addon_name, addon_path, edition).await
    }

    else {
        get_download(game, group_name, addon_name, edition).await
    }
}

#[inline]
pub async fn get_download_addon_task(game_info: &CardInfo, addon: &Addon, group: &AddonsGroup) -> HeapResult<Box<DownloadDiffQueuedTask>> {
    let download_path = addon.get_installation_path(&group.name, game_info.get_name(), game_info.get_edition()).await
        .map_err(|err| Box::new(MainAppMsg::ShowToast {
